...
from tracker import init_db, print_dashboard, upsert_job
...
def run_pipeline(skip_scrape: bool = False, cover_n: int = 5, limit: int | None = None, rescore_model: str | None = None):
    ...
    log.info("Options: skip_scrape=%s  cover_n=%d  limit=%s", skip_scrape, cover_n, limit)
    ...
    for _, row in scored_df.iterrows():
        ...
        try:
            ...
            log.warning("DB upsert failed for %s / %s: %s", company, title, e)
            skipped += 1
    ...
    log.info("=" * 60)
    log.info("PIPELINE COMPLETE - total time: %dm %ds", elapsed // 60, elapsed % 60)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--no-scrape", action="store_true", help="Skip scraping; re-score existing raw CSV")
